...
@section('page-header')
@php
    $transferUser = auth()->user()->isAdmin() ? ($currentUser ?? auth()->user()) : $user;
@endphp
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h1 class="text-2xl font-bold text-white">საფულე</h1>
        <p class="text-dark-400 mt-1">ბალანსების მართვა</p>
    </div>
    <div class="flex items-center gap-2 flex-wrap">
        ...
        <div class="relative" x-data="transferDropdown({
            balance: {{ (float)($transferUser->balance ?? 0) }},
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
    <div class="stat-card">
        <p class="text-dark-400 text-sm">ჯამური ბალანსი</p>
        <p class="text-2xl font-bold text-green-400 mt-1">${{ number_format($totals['total_balance'], 2) }}</p>
    </div>
    <div class="stat-card">
        <p class="text-dark-400 text-sm">მომხმარებლები ბალანსით</p>
        <p class="text-2xl font-bold text-white mt-1">{{ $totals['users_with_balance'] }}</p>
    </div>
</div>
        ...
        <table class="w-full table-dark">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right text-green-400 font-medium text-lg">{{ $user->getFormattedBalance() }}</td>
                    <td class="text-right">
                        <a href="{{ route('wallet.show', $user) }}" class="text-primary-400 hover:text-primary-300">
                            დეტალები
                        </a>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="4" class="text-center py-8 text-dark-500">მომხმარებლები ვერ მოიძებნა</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
@php $transferUser = auth()->user()->isAdmin() ? ($currentUser ?? auth()->user()) : $user; @endphp
<!-- Transfer modals (same data as header) -->
<div x-data="transferModals()" x-cloak
     data-balance="{{ (float)($transferUser->balance ?? 0) }}"
    ...
    <div x-show="modalCarToCar" x-transition class="fixed inset-0 z-[100] flex items-center justify-center p-4" style="display: none;">
        ...
        <div class="relative z-10 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div class="glass-card p-6">
                ...
                <p class="text-dark-400 text-sm mb-4">გადატანა მხოლოდ პლიუს დეპოზიტიდან (გადახდილი − სრული ღირებულება).</p>
                <div class="space-y-4">
...
function transferDropdown(config) {
    return {
        open: false,
        ...config,
        openWalletToCarModal() {
            if (window.walletTransferModals) {
                window.walletTransferModals.openWalletToCar();
            }
        },
        openCarToCarModal() {
            if (window.walletTransferModals) {
                window.walletTransferModals.openCarToCar();
            }
        }
    };
}
function transferModals() {
    const state = {
        ...
        carToCarUrl: '',
        csrf: '',
        walletToCar: { car_id: '', amount: '' },
        carToCar: { from_car_id: '', to_car_id: '', amount: '' },
        maxCarToCar: 0,
        errorWallet: '',
        errorCarToCar: '',
        carSearchWalletOpen: false,
        carSearchWalletQuery: '',
        carSearchFromOpen: false,
        carSearchFromQuery: '',
        carSearchToOpen: false,
        carSearchToQuery: '',
        filterCars(list, q) {
            ...
            return list.filter(function(c) {
                var vin = (c.vin || '').toLowerCase();
                var name = (c.make_model || '').toLowerCase();
                return vin.indexOf(q) !== -1 || name.indexOf(q) !== -1;
            });
        },
        getCarLabel(id, list) {
            if (!id || !list) return '';
        ...
        filterCarsOverpayment(q) {
            ...
            return this.carsWithOverpayment.filter(function(c) {
                var vin = (c.vin || '').toLowerCase();
                var name = (c.make_model || '').toLowerCase();
                return vin.indexOf(q) !== -1 || name.indexOf(q) !== -1;
            });
        ...
        init() {
            ...
            window.addEventListener('open-wallet-to-car-modal', () => {
                if (window.walletTransferModals) {
                    window.walletTransferModals.openWalletToCar();
                }
            });
            window.addEventListener('open-car-to-car-modal', () => {
                if (window.walletTransferModals) {
                    window.walletTransferModals.openCarToCar();
                }
            });
        ...
        openWalletToCar() {
            this.errorWallet = '';
            this.walletToCar = { car_id: '', amount: '' };
            this.carSearchWalletOpen = false;
            this.carSearchWalletQuery = '';
            this.modalWalletToCar = true;
        },
        openCarToCar() {
            this.errorCarToCar = '';
            this.carToCar = { from_car_id: '', to_car_id: '', amount: '' };
            this.maxCarToCar = 0;
            this.carSearchFromOpen = false;
            this.carSearchFromQuery = '';
            this.carSearchToOpen = false;
            this.carSearchToQuery = '';
            this.modalCarToCar = true;
        },
        updateMaxCarToCar() {
        ...
        async submitWalletToCar() {
            ...
            const res = await fetch(this.walletToCarUrl, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': this.csrf },
                body: JSON.stringify({ car_id: this.walletToCar.car_id, amount: amount, _token: this.csrf })
            });
        ...
        async submitCarToCar() {
            ...
            const res = await fetch(this.carToCarUrl, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': this.csrf },
                body: JSON.stringify({ from_car_id: this.carToCar.from_car_id, to_car_id: this.carToCar.to_car_id, amount: amount, _token: this.csrf })
            });
            const data = await res.json();
