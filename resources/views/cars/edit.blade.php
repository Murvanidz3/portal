...
@section('content')
    ...
    <form method="POST" action="{{ route('cars.update', $car) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="glass-card p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="make" id="make" value="{{ $carMake }}" class="form-input w-full" required>
                    @error('make')
                ...
                <div>
                    ...
                    <input type="text" name="model" id="model" value="{{ $carModel }}" class="form-input w-full" required>
                    @error('model')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="auction_name" id="auction_name" class="form-input w-full" required>
                        ...
                        <option value="COPART" {{ old('auction_name', $car->auction_name) == 'COPART' ? 'selected' : '' }}>
                            COPART</option>
                        <option value="IAAI" {{ old('auction_name', $car->auction_name) == 'IAAI' ? 'selected' : '' }}>IAAI
                        </option>
                        <option value="Manheim" {{ old('auction_name', $car->auction_name) == 'Manheim' ? 'selected' : '' }}>
                            Manheim</option>
                        <option value="Other" {{ old('auction_name', $car->auction_name) == 'Other' ? 'selected' : '' }}>სხვა
                        </option>
                    </select>
                    @error('auction_name')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" name="purchase_date" id="purchase_date" autocomplete="off"
                        value="{{ old('purchase_date', $car->purchase_date?->format('Y-m-d')) }}" class="form-input w-full date-input" placeholder="dd/mm/yyyy">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="arrival_date" id="arrival_date" autocomplete="off"
                        value="{{ old('arrival_date', $car->arrival_date?->format('Y-m-d')) }}" class="form-input w-full date-input" placeholder="dd/mm/yyyy">
                </div>
                ...
                <div>
                    <label for="document_received_at" class="block text-sm font-medium text-dark-300 mb-2">საწყობში საბუთის
                        მიღება</label>
                    <input type="text" name="document_received_at" id="document_received_at" autocomplete="off"
                        value="{{ old('document_received_at', $car->document_received_at?->format('Y-m-d')) }}"
                ...
                <div>
                    <label for="document_issued_at" class="block text-sm font-medium text-dark-300 mb-2">საბუთის გაცემის
                        თარიღი</label>
                    <input type="text" name="document_issued_at" id="document_issued_at" autocomplete="off"
                        value="{{ old('document_issued_at', $car->document_issued_at?->format('Y-m-d')) }}"
                        class="form-input w-full date-input" placeholder="dd/mm/yyyy">
                </div>
                ...
                <!-- Status -->
                <div>
                    <label for="status" class="block text-sm font-medium text-dark-300 mb-2">სტატუსი <span
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="container_number" id="container_number"
                        value="{{ old('container_number', $car->container_number) }}" class="form-input w-full uppercase">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="booking_number" id="booking_number"
                        value="{{ old('booking_number', $car->booking_number) }}" class="form-input w-full">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="shipping_line" id="shipping_line"
                        value="{{ old('shipping_line', $car->shipping_line) }}" class="form-input w-full">
                </div>

                <!-- Auction Location (გამშვები პორტი) -->
                <div>
                    <label for="auction_location" class="block text-sm font-medium text-dark-300 mb-2">გამშვები
                        პორტი</label>
                    <input type="text" name="auction_location" id="auction_location"
                        value="{{ old('auction_location', $car->auction_location) }}" class="form-input w-full"
                        placeholder="მაგ: CA-LOS ANGELES">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="loading_date" id="loading_date" autocomplete="off"
                        value="{{ old('loading_date', $car->loading_date?->format('Y-m-d')) }}" class="form-input w-full date-input" placeholder="dd/mm/yyyy">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="estimated_arrival_date" id="estimated_arrival_date" autocomplete="off"
                        value="{{ old('estimated_arrival_date', $car->estimated_arrival_date?->format('Y-m-d')) }}"
                        class="form-input w-full date-input" placeholder="dd/mm/yyyy">
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="dealer_phone" id="dealer_phone"
                        value="{{ old('dealer_phone', $car->dealer_phone) }}" class="form-input w-full">
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="vehicle_cost" id="vehicle_cost"
                        value="{{ old('vehicle_cost', $car->vehicle_cost) }}" class="form-input w-full" step="0.01" min="0"
                        required>
                    <p class="text-xs text-dark-500 mt-1">ავტომობილის ღირებულება საკომისიოს ჩათვლით</p>
                    @error('vehicle_cost')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- Additional Cost -->
                <div>
                    <label for="additional_cost" class="block text-sm font-medium text-dark-300 mb-2">დამატებითი ხარჯები
                        ($)</label>
                    <input type="number" name="additional_cost" id="additional_cost"
                        value="{{ old('additional_cost', $car->additional_cost ?? 0) }}" class="form-input w-full"
                        step="0.01" min="0">
                    <p class="text-xs text-dark-500 mt-1">ჯამურად დაემატება ტრანსპორტირების ხარჯთან ერთად</p>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="shipping_cost" id="shipping_cost"
                        value="{{ old('shipping_cost', $car->shipping_cost) }}" class="form-input w-full" step="0.01"
                        min="0" required>
                    @error('shipping_cost')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="border-2 border-dashed rounded-lg p-8 text-center transition-colors"
                ...
                <label for="photos" class="cursor-pointer">
                    <template x-if="selectedCount === 0">
                        <div>
                            <svg class="w-12 h-12 mx-auto text-dark-500 mb-4" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <template x-if="selectedCount > 0">
                        <div>
                            <svg class="w-12 h-12 mx-auto text-primary-500 mb-4" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                        ...
                        @foreach($car->files as $file)
                            <div class="relative aspect-video bg-dark-800 rounded-lg overflow-hidden group">
                                ...
                                <div
                                    ...
                                    <button type="button" onclick="setMainPhoto({{ $file->id }})"
                                        class="p-1 bg-primary-500 rounded text-white text-xs" title="მთავარი ფოტო">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M5 13l4 4L19 7" />
                                        </svg>
                                    </button>
                                    <button type="button" onclick="deletePhoto({{ $file->id }})"
                                        class="p-1 bg-red-500 rounded text-white text-xs" title="წაშლა">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="glass-card p-6">
            ...
            <textarea name="notes" id="notes" rows="3"
                class="form-input w-full resize-none">{{ old('notes', $car->notes) }}</textarea>
        </div>
    ...
    @if(auth()->user()->isAdmin())
        <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm hidden"
            style="display: none;">
