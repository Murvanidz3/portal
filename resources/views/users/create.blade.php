...
<div class="flex items-center gap-4">
    ...
    <h1 class="text-2xl font-bold text-white">მომხმარებლის დამატება</h1>
</div>
@endsection
    ...
    <div class="glass-card p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="username" value="{{ old('username') }}" class="form-input w-full" required>
            ...
            <div>
                ...
                <input type="text" name="full_name" value="{{ old('full_name') }}" class="form-input w-full">
            </div>
            ...
            <div>
                <label class="block text-sm font-medium text-dark-300 mb-2">პაროლი *</label>
            ...
            <div>
                ...
                <select name="role" class="form-input w-full" required>
                    @foreach($roles as $key => $label)
                        <option value="{{ $key }}" {{ old('role', 'dealer') == $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="number" name="balance" value="{{ old('balance', 0) }}" class="form-input w-full" step="0.01" min="0">
            </div>
            
            <div class="md:col-span-2 flex items-center gap-6">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="sms_enabled" value="1" {{ old('sms_enabled') ? 'checked' : '' }}
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="approved" value="1" {{ old('approved', true) ? 'checked' : '' }}
                           class="w-4 h-4 rounded border-dark-600 bg-dark-800 text-primary-600 focus:ring-primary-500">
