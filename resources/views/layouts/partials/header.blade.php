...
<header class="sticky top-0 z-30 bg-dark-900/80 backdrop-blur-xl border-b border-white/5">
    <div class="flex items-center justify-between h-16 px-4 lg:px-6">
        ...
        <div class="flex items-center">
            {{-- Mobile Menu Button (hidden for clients - no sidebar) --}}
        ...
        <div class="flex items-center space-x-3">
            ...
            <button id="theme-toggle" onclick="toggleTheme()"
                class="p-2 rounded-lg text-dark-400 hover:text-white hover:bg-dark-800 transition-all duration-300"
                title="თემის შეცვლა">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open"
                    ...
                    @if(auth()->user()->getUnreadNotificationsCount() > 0)
                        <span
                            class="absolute top-0 right-0 w-5 h-5 flex items-center justify-center text-xs font-bold text-white bg-red-500 rounded-full">
                ...
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200"
                        ...
                        @forelse(auth()->user()->notifications()->latest()->take(5)->get() as $notification)
                            <a href="{{ route('notifications.index') }}"
                                ...
                                <div class="flex items-start">
                                    <div
                                        class="w-8 h-8 rounded-full bg-primary-500/20 flex items-center justify-center flex-shrink-0 mr-3">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p
                                            class="text-sm text-white {{ !$notification->is_read ? 'font-medium' : '' }} line-clamp-2">
                    ...
                    <a href="{{ route('notifications.index') }}"
                        class="block px-4 py-3 text-center text-sm font-medium text-primary-400 hover:text-primary-300 border-t border-white/5">
                        ყველას ნახვა
                    </a>
                </div>
            </div>
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open"
                    ...
                    <div
                        class="w-8 h-8 rounded-full bg-gradient-to-br from-primary-500 to-primary-700 flex items-center justify-center text-white font-semibold text-sm">
                        {{ strtoupper(substr(auth()->user()->full_name ?? auth()->user()->username, 0, 1)) }}
                    </div>
                    <div class="hidden md:block text-left">
                        <p class="text-sm font-medium text-white">
                            {{ auth()->user()->full_name ?? auth()->user()->username }}</p>
                        <p class="text-xs text-dark-400 capitalize">{{ auth()->user()->role_label }}</p>
                    </div>
                    <svg class="w-4 h-4 text-dark-400 hidden md:block" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                ...
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div class="py-2">
                        <a href="{{ route('profile.edit') }}"
                            class="flex items-center px-4 py-2 text-sm text-dark-300 hover:text-white hover:bg-dark-700/50 transition-colors">
                    ...
                    <div class="py-2 border-t border-white/5">
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit"
                                class="w-full flex items-center px-4 py-2 text-sm text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-colors">
                                <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
