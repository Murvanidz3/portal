        ...
        <div class="space-y-4">
            ...
            <div class="p-4 rounded-lg bg-dark-800/50">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-sm font-medium text-primary-400">{{ $template->description ?? $template->status_key }}</span>
                    <button onclick="editTemplate({{ $template->id }}, '{{ $template->status_key }}')" 
                            class="text-sm text-dark-400 hover:text-primary-400">
                        რედაქტირება
                    </button>
                </div>
                <p class="text-sm text-dark-300" id="template-text-{{ $template->id }}">{{ $template->template_text }}</p>
            </div>
            @endforeach
        </div>
    </div>
